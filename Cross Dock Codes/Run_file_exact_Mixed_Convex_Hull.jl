...
println("Method Used - Mixed_Convex_Hull");

println("Inbound - $(n_inbound), Outbound - $(n_outbound), Doors - $(n_doors)");


function Temp_Storage_Mixed_Exact_plot(Enter_IT,Enter_OT,Leave_IT,Leave_OT,Load_Tran6)
    ...
    savefig("Temp Storage Utilized-Exact-Convex Hull-Mixed Doors");
    
end;

Enter_IT = zeros(Float64, n_inbound, n_doors);
Leave_IT = zeros(Float64, n_inbound, n_doors);    
Enter_OT = zeros(Float64, n_outbound, n_doors);
Leave_OT = zeros(Float64, n_outbound, n_doors);
Load_Tran6 = zeros(Int32, n_inbound, n_outbound, n_products);


include("code_Mixed_Convex_Hull.jl");
...
if (termination_status(CDSP) == MOI.OPTIMAL) || ((termination_status(CDSP) == MOI.TIME_LIMIT) && has_values(CDSP))
    ...
    #include("schedule_gantt_exact_Mixed_Convex_Hull.jl");

    resultfile = open("Solution_file_exact_Mixed_Convex_Hull.csv", "w");
    println(resultfile, "Trailer_num, Trailer_type, Arrival_time, Deadline, n_doors, n_inbound, n_outbound, n_products, Change_time, Supply, Demand, Penalty, BP1, BP2, M1, M2, M3, Assigned_DD, DD_entry_time, DD_leave_time, Tardiness, Objective value, Termination Status , Solution_time");
    for i in 1:n_inbound
        door_assign = sum(k for k = 1:n_doors if value(InbAssign[i, k]) == 1);
        println(resultfile, "IT$(i), Inbound trailer, $(Arrival[i]), NA, $(n_doors), $(n_inbound), $(n_outbound), $(n_products), $(Change_time), $(sum(Supply[i,p] for p = 1:n_products)), NA, NA, $(Breakpt[1]), $(Breakpt[2]), $(Multiplier[1]), $(Multiplier[2]), $(Multiplier[3]), Strip Door $(door_assign), $(value(EntryInbound[i])), $(value(LeaveInbound[i])), NA, $(objective_value(CDSP)), $(termination_status(CDSP)), $(sol_time)");
        Enter_IT[i,door_assign] = value(EntryInbound[i]);
        Leave_IT[i,door_assign] = value(EntryInbound[i])+sum(Supply[i,:].*UnLoadTime);
    end;
    for j in 1:n_outbound
        door_assign = sum(g for g = 1:n_doors if value(OutAssign[j, g]) == 1);
        println(resultfile, "OT$(j), Outbound trailer, $(0), $(DeadLines[j]), $(n_doors), $(n_inbound), $(n_outbound), $(n_products), $(Change_time), NA, $(sum(Demand[j,p] for p = 1:n_products)), $(Penalty[j]), $(Breakpt[1]), $(Breakpt[2]), $(Multiplier[1]), $(Multiplier[2]), $(Multiplier[3]), Stack Door $(door_assign), $(value(EntryOutbound[j])), $(value(LeaveOutbound[j])), $(value(Tardiness[j])), $(objective_value(CDSP)), $(termination_status(CDSP)), $(sol_time)");
        Enter_OT[j,door_assign] = value(EntryOutbound[j]);
        Leave_OT[j,door_assign] = value(LeaveOutbound[j]);
    end;
    close(resultfile);

    resultfile = open("Flows_exact_Mixed_Convex_Hull.csv", "w");
    println(resultfile, "Inbound Trailer, Outbound Trailer, Product, Quantity");
    for i in 1:n_inbound
        str_dd = sum(k for k = 1:n_doors if value(InbAssign[i, k]) == 1);
        for j in 1:n_outbound
            sta_dd = sum(g for g = 1:n_doors if value(OutAssign[j, g]) == 1);
            for p in 1:n_products
    ...
    close(resultfile);
    Temp_Storage_Mixed_Exact_plot(Enter_IT,Enter_OT,Leave_IT,Leave_OT,Load_Tran6);
