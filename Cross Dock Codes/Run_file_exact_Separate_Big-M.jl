...
#=
n_strip = Int(round((n_inbound*n_doors/(n_inbound+n_outbound))));
n_stack = n_doors-n_strip;
=#
...
println("Method Used - Exact");
println("Inbound - $(n_inbound), Outbound - $(n_outbound), Doors - $(n_strip)_$(n_stack)");
...
function Temp_Storage_Exact_Separate_plot(Enter_IT,Enter_OT,Leave_IT,Leave_OT,Load_Tran6)
    ...
    savefig("Temp Storage Utilized-Exact-Separate Doors");
...
include("code_Separate_Big-M.jl");
#include("code_v1.8_Convex_Hull.jl");
...
if (termination_status(CDSP) == MOI.OPTIMAL) || ((termination_status(CDSP) == MOI.TIME_LIMIT) && has_values(CDSP))
    ...
    #include("schedule_gantt_exact.jl");

    resultfile = open("Solution_file_exact_$(n_inbound)_$(n_outbound)_$(n_strip)_$(n_stack).csv", "w");
    ...
    resultfile = open("In_Precedence_$(n_inbound)_$(n_outbound)_$(n_strip)_$(n_stack).csv", "w");
    println(resultfile, "Inbound Trailer, Inbound Trailer, Value");
    for i in 1:n_inbound
        for j in 1:n_inbound
            if value(InbPrecedence[i, j]) > 0.9
                println(resultfile, "$(i), $(j), $(value(InbPrecedence[i, j]))");
            end;
    ...
    resultfile = open("Out_Precedence_$(n_inbound)_$(n_outbound)_$(n_strip)_$(n_stack).csv", "w");
    println(resultfile, "Outbound Trailer, Outbound Trailer, Value");
    for i in 1:n_outbound
        for j in 1:n_outbound
            if value(OutPrecedence[i, j]) > 0.9
                println(resultfile, "$(i), $(j), $(value(OutPrecedence[i, j]))");
            end;
    ...
    resultfile = open("Flows_exact_$(n_inbound)_$(n_outbound)_$(n_strip)_$(n_stack).csv", "w");
