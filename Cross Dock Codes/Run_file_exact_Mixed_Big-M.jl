...
function Temp_Storage_Mixed_Exact_plot(Enter_IT,Enter_OT,Leave_IT,Leave_OT,Load_Tran6)
    ...
    savefig("Temp Storage Utilized-Exact-Mixed Doors");
...
if (termination_status(CDSP) == MOI.OPTIMAL) || ((termination_status(CDSP) == MOI.TIME_LIMIT) && has_values(CDSP))
    ...
    #include("schedule_gantt_exact_Mixed.jl");

    resultfile = open("Solution_file_exact_Mixed.csv", "w");
    ...
    for i in 1:n_inbound
        door_assign = sum(k for k = 1:n_doors if value(InbAssign[i, k]) > 0.9);
    ...
    for j in 1:n_outbound
        door_assign = sum(g for g = 1:n_doors if value(OutAssign[j, g]) > 0.9);
    ...
    resultfile = open("Flows_exact_Mixed.csv", "w");
